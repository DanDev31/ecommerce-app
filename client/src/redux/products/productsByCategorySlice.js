    ...
    try {
      if (queriesArray === undefined || queriesArray.length === 0) {
        ...
        if (queriesArray.length > 0) {
          ...
          if (hasFalseValue) {
            const response = await axiosInstance.get(
              `/products/?category=${savedCategory}&${queriesArray
            ...
            if (onlyTrueValues.length > 0) {
              const response = await axiosInstance.get(
                `/products/?category=${savedCategory}&${onlyTrueValues
                  .map((brand) => `brandValue=${brand.brandValue}`)
                  .join("&")}`
              );
              return response.data;
...
const produtsByCategorySlice = createSlice({
  ...
  reducers: {
    orderByPrice(state, { payload }) {
      if (payload === "low") {
        state.productsByCategory = state.productsByCategory.sort(function (
          a,
          b
        ) {
          if (a.price > b.price) {
      ...
      if (payload === "high") {
        state.productsByCategory = state.productsByCategory
          .sort(function (a, b) {
            if (a.price > b.price) {
              return 1;
            }
